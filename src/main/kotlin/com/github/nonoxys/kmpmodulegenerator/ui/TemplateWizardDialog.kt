...
class TemplateWizardDialog(private val project: Project) : DialogWrapper(project) {
    ...
    private fun addStandardParameter(name: String, displayName: String, type: String, required: Boolean, default: String) {
    ...
    private fun addParameter() {
        ...
        if (dialog.showAndGet()) {
            ...
            if (parameters.any { it.name == param.name }) {
                Messages.showErrorDialog(project, "Parameter with name '${param.name}' already exists", "Duplicate Parameter")
                return
            }
